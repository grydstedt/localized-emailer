...
LocalizedEmailer.prototype.email = function(name, data, opts, cb) {
  ...
  opts.text = email.text ? ejs.render(email.text, data) : (opts.text || '');
  opts.html = email.html ? ejs.render(email.html, data) : (opts.html || '');
  opts.subject = email.subject ? ejs.render(email.subject, data) : (opts.subject || '');
...
LocalizedEmailer.prototype.readDirectory = function(dirName, cb) {
  ...
  var findLocales = function(type, emailName, emailDirName, inCb) {
    ...
    fs.readdir(dirName, function(err, files) {
      ...
      if(err) {
        console.warn('no folder ' + type + ' found for email ' + emailName);
        return inCb();
      }
